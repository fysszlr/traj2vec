...
def train(args):
    ...
    with open("log.out","a") as file:
        print("Reading training data...",file=file)
    trainData.load(args.max_num_line)
    print("Allocation: {}".format(trainData.allocation))
    with open("log.out","a") as file:
        print("Allocation: {}".format(trainData.allocation),file=file)
    print("Percent: {}".format(trainData.p))
    with open("log.out","a") as file:
        print("Percent: {}".format(trainData.p),file=file)
    ...
    if args.criterion_name == "NLL":
        ...
        with open("log.out","a") as file:
            print("Loading vocab distance file {}...".format(args.knearestvocabs),file=file)
    ...
    if args.cuda and torch.cuda.is_available():
        ...
        with open("log.out","a") as file:
            print("=> training with GPU",file=file)
        m0.cuda()
        m1.cuda()
        criterion.cuda()
        ...
        with open("log.out","a") as file:
            print("=> training with CPU",file=file)
    ...
    if os.path.isfile(args.checkpoint):
        ...
        with open("log.out","a") as file:
            print("=> loading checkpoint '{}'".format(args.checkpoint),file=file)
        ...
        with open("log.out","a") as file:
            print("=> no checkpoint found at '{}'".format(args.checkpoint),file=file)
    ...
    with open("log.out","a") as file:
        print("Iteration starts at {} "
          "and will end at {}".format(args.start_iteration, num_iteration-1),file=file)
    ## training
        ...
        try:
            ...
            if iteration % args.print_freq == 0:
                ...
                with open("log.out","a") as file:
                    print("Iteration: {0:}\tGenerative Loss: {1:.3f}\t"\
                      "Discriminative Cross Loss: {2:.3f}\tDiscriminative Inner Loss: {3:.3f}"\
                      .format(iteration, avg_genloss, disloss_cross, disloss_inner),file=file)
            if iteration % args.save_freq == 0 and iteration > 0:
                ...
                with open("log.out","a") as file:
                    print("Saving the model at iteration {} validation loss {}"\
                      .format(iteration, prec_loss),file=file)
                savecheckpoint({
                    "iteration": iteration,
